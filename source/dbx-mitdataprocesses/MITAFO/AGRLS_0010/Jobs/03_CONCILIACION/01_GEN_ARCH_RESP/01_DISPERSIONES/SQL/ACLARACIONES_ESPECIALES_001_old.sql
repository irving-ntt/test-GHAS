SELECT /*+ parallel(8) */   -- ACLARACIONES ESPECIALES
         ...
         D.FTN_NO_LINEA,
         --D.FNN_ID_REFERENCIA,
         -- Condicional para FCN_ID_SIEFORE
         CASE WHEN D.FCN_ID_SIEFORE IS NULL THEN 846 ELSE D.FCN_ID_SIEFORE END FCN_ID_SIEFORE,
         D.FCN_ID_TIPO_SUBCTA,
         ROUND((CASE WHEN CCM.FCN_ID_TIPO_MONTO = 184 THEN D.FTF_MONTO_PESOS ELSE 0 END), 6) FTF_MONTO_PESOS,
         ROUND((CASE WHEN CCM.FCN_ID_TIPO_MONTO = 199 THEN D.FTF_MONTO_ACCIONES ELSE 0 END), 6) FTF_MONTO_ACCIONES,
         D.FTC_TABLA_NCI_MOV,
         D.FNN_ID_VIV_GARANTIA as FTN_ID_CRED_VIV,
         D.FFN_ID_CONCEPTO_MOV,
         --D.FCN_ID_PROCESO,
         --D.FCN_ID_SUBPROCESO,
         --NVL(D.FTN_NSS, 0) FTN_NSS,
         --NVL(D.FTC_CURP, 0) FTC_CURP,
         D.FTN_ID_MONTO,
         D.FTN_TIPO_REG,
         --VMC.FTN_NUM_CTA_INVDUAL, -- Campo adicional desde la tabla filtrada
         -- Condicional para FTN_ESTATUS
         CASE WHEN D.FTN_NUM_CTA_INVDUAL IS NOT NULL AND VMC.FTB_ESTATUS_MARCA IN (1, 2) THEN 1 ELSE 0 END FTN_ESTATUS
  FROM (SELECT * FROM CIERREN_ETL.TTSISGRAL_ETL_DISPERSION WHERE FFN_ID_CONCEPTO_MOV <> 231 AND FTC_FOLIO = '#SR_FOLIO#') D
  ...
  LEFT JOIN (
        ...
        WHERE FTC_FOLIO = '#SR_FOLIO#' -- Filtración previa por SRC_FOLIO
