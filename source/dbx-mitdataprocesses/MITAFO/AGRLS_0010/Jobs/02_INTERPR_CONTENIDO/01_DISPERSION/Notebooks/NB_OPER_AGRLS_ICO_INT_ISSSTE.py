...
statement = query.get_statement(
    "INCO_INTISSSTE_EXT_0100_OCI_LEE_ARCHIVO.sql",
...
statement = query.get_statement(
    "INCO_INTISSSTE_TRN_0400_DBK_PRE_DIS.sql",
...
statement = query.get_statement(
    "INCO_INTISSSTE_TRN_0500_DBK_PRE_DIS.sql",
    sr_folio=params.sr_folio,
    sr_proceso = params.sr_proceso,
    sr_subproceso = params.sr_subproceso,
    DELTA_TABLA_NAME1 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_INCO_01_{params.sr_folio}"
...
statement = query.get_statement(
    "INCO_INTISSSTE_TRN_0700_DBK_PRE_DIS.sql",
...
db.write_delta(f"DELTA_INCO_065_{params.sr_folio}", db.sql_delta(statement), "overwrite")
if conf.debug:
    display(db.read_delta(f"DELTA_INCO_065_{params.sr_folio}"))

# COMMAND ----------

#Query  --> PRE DISPERSION

statement = query.get_statement(
    "INCO_INTISSSTE_TRN_0800_DBK_PRE_DIS.sql",
    DELTA_TABLA_NAME7 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_INCO_065_{params.sr_folio}"
...
statement = query.get_statement(
    "INCO_INTISSSTE_TRN_0900_DBK_PRE_DIS.sql",
    DELTA_TABLA_NAME7 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_INCO_065_{params.sr_folio}"

)

#El contenido de esta tabla delta se va a insertar en PRE DISPERSION
db.write_delta(f"DELTA_INCO_INT_TOT_{params.sr_folio}", db.sql_delta(statement), "overwrite")

if conf.debug:
    display(db.read_delta(f"DELTA_INCO_INT_TOT_{params.sr_folio}"))
