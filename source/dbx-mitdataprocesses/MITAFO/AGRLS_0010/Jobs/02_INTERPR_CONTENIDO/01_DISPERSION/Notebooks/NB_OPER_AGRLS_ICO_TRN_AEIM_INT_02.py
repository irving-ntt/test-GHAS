...
statement = query.get_statement(
    "INCO_AEIM_TRN_400_DBK_INT_PRE_DIS.sql",
...
statement = query.get_statement(
    "INCO_AEIM_TRN_500_DBK_INT_CTE_SUA.sql",
    sr_folio=params.sr_folio,
    sr_proceso = params.sr_proceso,
    sr_subproceso = params.sr_subproceso,
    DELTA_TABLA_NAME1 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_INCO_01_{params.sr_folio}"
...
#Query Transformación --> Tabla TTSISGRAL_ETL_AEIM

statement = query.get_statement(
    "INCO_AEIM_TRN_800_DBK_INT_LEE_ARCHIVO.sql",
    sr_folio=params.sr_folio,
    DELTA_TABLA_NAME4 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_INCO_08_{params.sr_folio}"
)

#El contenido de esta tabla delta se va a insertar en PRE DISPERSION
db.write_delta(f"DELTA_INCO_09_{params.sr_folio}", db.sql_delta(statement), "overwrite")

if conf.debug:
    display(db.read_delta(f"DELTA_INCO_09_{params.sr_folio}"))
