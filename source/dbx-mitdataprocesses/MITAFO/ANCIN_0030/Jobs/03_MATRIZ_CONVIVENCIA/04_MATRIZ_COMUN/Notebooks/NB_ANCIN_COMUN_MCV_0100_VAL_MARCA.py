...
# DBTITLE 1,EXTRAE INFORMACION DE MATRIZ
...
# DBTITLE 1,EXTRAE CONFIG CONVIV
# Query Extrae informacion de config conviv se extrae mas adelante
...
#db.write_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_03_CONFIG_CONVIV_{params.sr_folio}", db.read_data("default", statement), "overwrite")

#if conf.debug:
#    display(db.read_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_03_CONFIG_CONVIV_{params.sr_folio}"))
...
# 
statement = query.get_statement(
    "COMUN_MCV_100_TD_0100_GEN_MATRIZ_DELTA_PRE_Y_MATRIZ.sql",
    sr_folio=params.sr_folio,
...
db.write_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_01_PRE_Y_MATRIZ_{params.sr_folio}", db.sql_delta(statement), "overwrite")

if conf.debug:
    display(db.read_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_01_PRE_Y_MATRIZ_{params.sr_folio}"))
...
db.write_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_03_CONFIG_CONVIV_{params.sr_folio}", db.read_data("default", statement), "overwrite")
#                TEMP_DELTA_COMUN_MCV_DELTA_01_PRE_MAT_
...
# UNIMOS PRE Y MATRIZ con CONFIG CONVIV
statement = query.get_statement(
    "COMUN_MCV_100_TD_0100_GEN_MATRIZ_PRE_Y_MATRIZ_CC.sql",
    sr_folio=params.sr_folio,
    DELTA_TABLA_NAME1 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.TEMP_DELTA_COMUN_MCV_DELTA_01_PRE_Y_MATRIZ_{params.sr_folio}",
    DELTA_TABLA_NAME2 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.TEMP_DELTA_COMUN_MCV_DELTA_03_CONFIG_CONVIV_{params.sr_folio}",
)
