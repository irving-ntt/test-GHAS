...
# MAGIC %md
# MAGIC NB_ANCIN_COMUN_MCV_0120_CARGA_VAL_MATRIZ_ABONO - 20251008
...
statement = query.get_statement(
    "COMUN_MCV_130_TD_05_EXT_INFO_A_DELTA_MATRIZ_ABONO.sql",
    sr_folio=params.sr_folio,
)

db.write_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_07_PRE_VAL_MATRIZ_ABONO_{params.sr_folio}", db.read_data("default", statement), "overwrite")

if conf.debug:
    display(db.read_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_07_PRE_VAL_MATRIZ_ABONO_{params.sr_folio}"))
#  -------------------------------------------------------------------------------------
...
# DBTITLE 1,VERIFICAMOS TABLA A CARGAR
if conf.debug:
    display(db.read_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_07_PRE_VAL_MATRIZ_ABONO_{params.sr_folio}"))
...
db.write_data(db.read_delta(f"TEMP_DELTA_COMUN_MCV_DELTA_07_PRE_VAL_MATRIZ_ABONO_{params.sr_folio}"), table_name, "default", "append")
...
statement = query.get_statement(
    "COMUN_MCV_130_TD_07_DB_MERGE_MATRIZ_ABONO.sql",
    SR_FOLIO=params.sr_folio,
    hints="/*+ PARALLEL(8) INDEX(TTAFOGRAL_ETL_MATRIZ_CONVIVENCIA_AUX) */",
)
execution = db.execute_oci_dml(
...
# DBTITLE 1,ELIMINAMOS DE LA TABLA AUXILIAR
# ELIMINA REGISTROS DEL FOLIO EN MATRIZ_AUX
statement = query.get_statement(
    "COMUN_MCV_130_TD_06_DEL_MATRIZ_AUX.sql",
    SR_FOLIO=params.sr_folio,
