...
'''
Descripcion:
    Validacion de estructura traspasos
Subetapa:
...
delta_001 = f"delta_001_{params.sr_folio}"
...
statement_demo_001 = query.get_statement(
    "valid_struct_0001_traspasos_INTGRTY.sql",
    SR_FOLIO=params.sr_folio,
    SR_SUBETAPA=params.sr_subetapa,
    MAX_LENGTH=conf.val_records_lenght_tras_65,
...
delta_002 = f"delta_002_{params.sr_folio}"
...
full_file_name = (
    SETTINGS.GENERAL.EXTERNAL_LOCATION + SETTINGS.GENERAL.PATH_INTEGRITY + "/" + SETTINGS.GENERAL.PATH_INF + "/" + params.sr_folio + "_" + conf.output_file_name_001 + ".csv"
)
...
if len(dfLoaded3.take(1)) > 0:
    dbutils.fs.mkdirs(SETTINGS.GENERAL.EXTERNAL_LOCATION + SETTINGS.GENERAL.PATH_INTEGRITY + '/Error/')
    dbutils.fs.mv(SETTINGS.GENERAL.EXTERNAL_LOCATION + params.sr_path_arch, SETTINGS.GENERAL.EXTERNAL_LOCATION + SETTINGS.GENERAL.PATH_INTEGRITY + "/Error/")
...
delta_003 = f"delta_003_{params.sr_folio}"
...
statement_demo_0004 = query.get_statement(
    "valid_struct_0004_traspasos_INTGRTY.sql",
    SR_FOLIO=params.sr_folio,
    SR_SUBETAPA=params.sr_subetapa,
    MAX_LENGTH=conf.val_records_lenght_tras_65,
...
dfLoaded4.createOrReplaceTempView(f"temp_view_002_{params.sr_folio}")
if conf.debug:
    display(dfLoaded4)
...
statement_demo_0005 = query.get_statement(
    "valid_struct_0005_traspasos.sql",
    temp_view_002 = f"temp_view_002_{params.sr_folio}",
)
...
statement_demo_006 = query.get_statement(
    "valid_struct_0006_traspasos_INTGRTY.sql",
    SR_FOLIO=params.sr_folio,
...
Notify.send_notification("VAL_ESTRUCT_TRAS", params)

# COMMAND ----------

db.drop_delta(f"delta_001_{params.sr_folio}")
db.drop_delta(f"delta_002_{params.sr_folio}")
db.drop_delta(f"delta_003_{params.sr_folio}")
