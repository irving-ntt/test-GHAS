# Databricks notebook source
 '''
Descripcion:
    Validacion de estructura ACESP
Subetapa:
...
Archivos SQL:
    valid_struct_0001_acesp.sql
    valid_struct_0003_acesp_2.sql
...
#df = spark.read.format("delta").load(f"delta_003_{params.sr_id_archivo}")
#df.write.mode("overwrite").csv(full_file_name)


df_read = spark.read.format("csv").option("header", "True").load(full_file_name)
if conf.debug:
    display(df_read)
...
delta_004 = f"delta_004_{params.sr_id_archivo}"
...
db.drop_delta(f"delta_004_{params.sr_id_archivo}")
