SELECT
	FTN_NO_LINEA
	,CASE WHEN LAG(substr(FTC_LINEA,5,8)) OVER(ORDER BY FTN_NO_LINEA) = '02' OR LEAD(substr(FTC_LINEA,5,8)) OVER(ORDER BY FTN_NO_LINEA) = '03' then NULL else substr(FTC_LINEA,5,8) end Error_en_Tipo_de_registro_diferente_a_03
	...
	,CASE WHEN substr(FTC_LINEA,5,8) < LEAD(substr(FTC_LINEA,5,8)) OVER(ORDER BY FTN_NO_LINEA) then NULL else substr(FTC_LINEA,5,8) end Error_en_formato_de_Consecutivo_de_registro_dentro_del_lote_invalido
	...
	,CASE WHEN substr(FTC_LINEA,172,3) != '' then NULL else substr(FTC_LINEA,172,3) end Error_clave_de_Entidad_Receptora_vacio
	...
	,CASE WHEN substr(FTC_LINEA,281,7) = '0000000' then NULL else substr(FTC_LINEA,281,7) end Error_Intereses_generados_en_Aclaraciones_por_Intereses_de_Pagos_Extemporaneos_de_Vivienda_invalido
	,CASE WHEN LENGTH(REGEXP_REPLACE(substr(FTC_LINEA,288,7), '[0-9]*', '')) = 0 then NULL else substr(FTC_LINEA,288,7) end Error_Importe_intereses_de_Aportaciones_a_Largo_Plazo_invalido
	,CASE WHEN LENGTH(REGEXP_REPLACE(substr(FTC_LINEA,295,7), '[0-9]*', '')) = 0 then NULL else substr(FTC_LINEA,295,7) end Interes_de_Aportacion_ASA_invalido
