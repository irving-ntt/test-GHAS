SELECT
  ...
  ,CASE WHEN substr(FTC_LINEA,1,2) = '01' then NULL else substr(FTC_LINEA,1,2) end Error_en_tipo_de_registro
	,CASE WHEN substr(FTC_LINEA,3,2) = '01' then NULL else substr(FTC_LINEA,3,2) end Error_en_Identificador_de_Servicio
	,CASE WHEN substr(FTC_LINEA,5,2) = '30' then NULL else substr(FTC_LINEA,5,2) end Error_en_indicador_de_operacion
	,CASE WHEN substr(FTC_LINEA,7,2) = '03' then NULL else substr(FTC_LINEA,7,2) end Error_en_tipo_de_entidad_origen
	,CASE WHEN substr(FTC_LINEA,9,3) = '001' then NULL else substr(FTC_LINEA,9,3) end Error_en_clave_de_entidad_origen
