...
SELECT
	FTN_NO_LINEA
	,CASE WHEN substr(FTC_LINEA,1,2) = '01' then NULL WHEN LENGTH(TRIM(substr(FTC_LINEA,1,2))) = 0 then 'Registro_Vacio' else substr(FTC_LINEA,1,2) end Tipo_de_Registro
  ...
  ,CASE WHEN substr(FTC_LINEA,17,3) = '009' then NULL WHEN LENGTH(TRIM(substr(FTC_LINEA,17,3))) = 0 then 'Registro_Vacio' else substr(FTC_LINEA,17,3) end Entidad_federativa_de_envio_de_lote
	...
	--,CASE WHEN substr(FTC_LINEA,28,3) = substring(regexp_extract('#NOMBRE_ARCHIVO#','_[0-9]{8}_[0-9]{3}_[A-Z]{6}.TXT$', 0), 11, 8) THEN NULL else substr(FTC_LINEA,28,3) end Folio_del_lote
