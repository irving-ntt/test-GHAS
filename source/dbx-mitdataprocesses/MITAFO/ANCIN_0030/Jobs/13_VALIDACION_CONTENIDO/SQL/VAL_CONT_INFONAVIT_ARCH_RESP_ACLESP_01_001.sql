SELECT
	...
	,CASE WHEN try_to_timestamp(substr(FTC_LINEA,17,8), 'yyyyMMdd') IS NOT NULL then NULL else substr(FTC_LINEA,17,8) end Error_en_Formato_de_fecha_erroneo_
	,CASE WHEN substr(FTC_LINEA,25,3) != '000' then NULL else substr(FTC_LINEA,25,3) end Error_en_Consecutivo_erroneo_000
	,CASE WHEN substr(FTC_LINEA,28,2) = '02' then NULL else substr(FTC_LINEA,28,2) end Error_en_Modalidad_de_recepcion_envio_del_archivo
	,CASE WHEN try_to_timestamp(substr(FTC_LINEA,30,8), 'yyyyMMdd') IS NOT NULL then NULL else substr(FTC_LINEA,30,8) end Error_en_Formato_de_fecha_erroneo
	,CASE WHEN substr(FTC_LINEA,320,1) = '2' then NULL else substr(FTC_LINEA,320,1) end Error_en_Diferente_a_2
