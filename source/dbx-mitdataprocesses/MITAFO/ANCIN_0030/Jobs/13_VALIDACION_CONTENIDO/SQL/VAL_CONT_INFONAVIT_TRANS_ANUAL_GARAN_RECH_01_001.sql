...
SELECT
  ...
  ,CASE WHEN substr(FTC_LINEA,17,3) = '009' then NULL WHEN LENGTH(TRIM(substr(FTC_LINEA,17,3))) = 0 then 'Registro_Vacio' else substr(FTC_LINEA,17,3) end Entidad_federativa_de_envio_de_lote
	...
	--,CASE WHEN substr(FTC_LINEA,28,3) = substring(regexp_extract(#NOMBRE_ARCHIVO#,'_[0-9]{8}_[0-9]{3}_[A-Z0-9]{10}.txt$', 0), -7, 3) else substr(FTC_LINEA,28,3) end Error_folio_del_lote
	--,CASE WHEN substr(FTC_LINEA,31,2) = REPEAT(' ', 2) then NULL else substr(FTC_LINEA,31,2) end FILLER
