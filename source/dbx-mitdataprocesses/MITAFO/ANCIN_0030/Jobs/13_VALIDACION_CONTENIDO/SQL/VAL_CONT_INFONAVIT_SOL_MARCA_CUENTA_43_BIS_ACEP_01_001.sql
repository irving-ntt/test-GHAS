...
SELECT
	...
	--,CASE WHEN substr(FTC_LINEA,20,8) = substring(regexp_extract('#NOMBRE_ARCHIVO#','_[0-9]{8}_[0-9]{3}_[A-Z0-9]{11}.TXT$', 0), 2, 8) THEN NULL else substr(FTC_LINEA,20,8) end Fecha_de_presentacion
	--,CASE WHEN substr(FTC_LINEA,28,3) = substring(regexp_extract('#NOMBRE_ARCHIVO#','_[0-9]{8}_[0-9]{3}_[A-Z0-9]{11}.TXT$', 0), 11, 3) THEN NULL else substr(FTC_LINEA,28,3) end Folio_del_lote
	--,CASE WHEN substr(FTC_LINEA,31,2) = REPEAT(' ', 2) then NULL else substr(FTC_LINEA,31,2) end Filler_01
	--,CASE WHEN substr(FTC_LINEA,33,2) = REPEAT(' ', 2) then NULL else substr(FTC_LINEA,33,2) end Codigo_resultado_de_la_Ope
	--,CASE WHEN substr(FTC_LINEA,35,9) = REPEAT(' ', 9) then NULL else substr(FTC_LINEA,35,9) end Error_en_Motivo_de_rechazo_de_lote
