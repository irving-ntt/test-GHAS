SELECT
  ...
  ,CASE WHEN substr(FTC_LINEA,1,2) = '09' then NULL else substr(FTC_LINEA,1,2) end Error_en_campo_Tipo_de_Registro_diferente_a_09
	...
	,CASE WHEN substr(FTC_LINEA,7,2) = '03' then NULL else substr(FTC_LINEA,7,2) end Error_en_campo_Tipo_entidad_origen_diferente_a_03
	...
	,CASE WHEN substr(FTC_LINEA,12,2) = '01' then NULL else substr(FTC_LINEA,12,2) end Error_en_campo_tipo_entidad_destino_diferente_a_01
	...
	,CASE WHEN LENGTH(REGEXP_REPLACE(substr(FTC_LINEA,25,10), '[0-9]*', '')) = 0 then NULL else substr(FTC_LINEA,25,10) end Error_en_campo_Total_de_registros_diferente_a_dato_numerico
