SELECT
	...
	,CASE WHEN substr(FTC_LINEA,7,2) = '03' then NULL else substr(FTC_LINEA,7,2) end Error_en_Tipo_entidad_origen_diferente_a_01
	...
	,CASE WHEN try_to_timestamp(substr(FTC_LINEA,30,8), 'yyyyMMdd') IS NOT NULL then NULL else substr(FTC_LINEA,30,8) end Error_en_Formato_de_Fecha_limite_de_respuesta
	,CASE WHEN substr(FTC_LINEA,320,1) = '1' then NULL else substr(FTC_LINEA,320,1) end Error_en_Diferente_a_1
