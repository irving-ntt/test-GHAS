...
--	END Error_de_Consecutivo_de_registro_dentro_del_lote_invalido
	...
	,CASE WHEN LENGTH(REGEXP_REPLACE(substr(FTC_LINEA,492,13), '[0-9 ]*', '')) = 0 then NULL else substr(FTC_LINEA,492,13) end Error_en_Numero_de_aplicaciones_de_Intereses_de_Vivienda_92
	,CASE WHEN LENGTH(REGEXP_REPLACE(substr(FTC_LINEA,567,13), '[0-9 ]*', '')) = 0 then NULL else substr(FTC_LINEA,567,13) end Error_en_Numero_de_aplicaciones_de_Intereses_de_vivienda_97
