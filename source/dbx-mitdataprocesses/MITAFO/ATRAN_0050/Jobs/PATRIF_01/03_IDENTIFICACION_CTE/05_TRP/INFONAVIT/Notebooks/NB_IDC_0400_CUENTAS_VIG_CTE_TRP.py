...
# DBTITLE 1,GENERA DELTA CURP
statement_3 = query.get_statement(
    "SQL_DELTA_400_CURP.sql",
    DELTA_TABLA_NAME=f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_400_UNION_{params.sr_folio}"
)

df_curp = db.sql_delta(statement_3)

if conf.debug:
    display(df_curp)

db.write_delta(f"DELTA_CURP_{params.sr_folio}", df_curp, "overwrite")

# COMMAND ----------

# DBTITLE 1,CREA DELTA ENCONTRADOS NSS
statement_3 = query.get_statement(
    "SQL_DELTA_400_UNION.sql",
    DELTA_TABLA_NAME=f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_400_UNION_{params.sr_folio}"
)

df_encontrados_nss = db.sql_delta(statement_3)

if conf.debug:
    display(df_encontrados_nss)

db.write_delta(f"DELTA_ENCONTRADOS_NSS_{params.sr_folio}", df_encontrados_nss, "overwrite")
