SELECT 
   ...
   BUC.FTN_CELULAR               ,
   BUC.FTC_CORREO_ELEC
FROM CIERREN_ETL.TTAFOTRAS_ETL_DESMARCA_FOVST CF
LEFT JOIN (
    SELECT * FROM (
      SELECT 
         CASE WHEN B.FTC_NSS IS NULL THEN B.FTC_CURP 
         ...
         B.FTN_CELULAR                                   ,
         B.FTC_CORREO_ELEC                               ,
         ROW_NUMBER() OVER (PARTITION BY B.FTC_CURP, B.FTN_NUM_CTA_INVDUAL ORDER BY B.FTC_CURP, B.FTN_NUM_CTA_INVDUAL ASC) AS RN
      FROM (
           SELECT FTC_CURP
           FROM CIERREN_ETL.TTAFOTRAS_ETL_DESMARCA_FOVST
...
ON CF.FTC_CURP = BUC.FTC_CURP

WHERE CF.FTN_ID_ARCHIVO = #SR_ID_ARCHIVO#
  AND CF.FTC_FOLIO = '#SR_FOLIO#'
