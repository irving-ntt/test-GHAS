...
statement = query.get_statement(
    "TRA_MCV_DEV_SALDO_EXCED_0300_EXT_INFO_REP.sql",
    sr_folio=params.sr_folio,
    sr_subproceso=params.sr_subproceso,
    sr_proceso=params.sr_proceso,
)

db.write_delta(f"DELTA_TRA_MCV_DEV_SALDOS_EXCED_{params.sr_folio}", db.read_data("default", statement), "overwrite")

if conf.debug:
    display(db.read_delta(f"DELTA_TRA_MCV_DEV_SALDOS_EXCED_{params.sr_folio}"))

# COMMAND ----------

# ELIMINA REGISTROS DEL FOLIO EN PRE MATRIZ
statement = query.get_statement(
    "TRA_MCV_DEV_SALDO_EXCED_0200_DEL_PRE_MATRIZ.sql",
    SR_FOLIO=params.sr_folio,
   sr_subproceso=params.sr_subproceso,
    hints="/*+ PARALLEL(8) */",
)
...
db.write_data(db.read_delta(f"DELTA_TRA_MCV_DEV_SALDOS_EXCED_{params.sr_folio}"), table_name, "default", "append")

# COMMAND ----------

db.drop_delta(f"DELTA_TRA_MCV_DEV_SALDOS_EXCED_{params.sr_folio}")
