...
'''
Descripcion:
    Creación de archivo CTINDI para Devolución de Pago sin Justificación Legal | Esta notebook corresponde a la estracción de los jobs: JP_PATRIF_CTINDI_0020_GEN_ARCHIVO_RCV
Subetapa:
...
Tablas OUTPUT DELTA:
    N/A
Archivos SQL:
    CTINDI_DPSJL_EXT_0400_OCI_GEN_ARCHIVO.sql
    CTINDI_DPSJL_TRN_0500_DBK.sql
    CTINDI_DPSJL_TRN_0600_DBK_TRAN_GEN_ARCHIVO.sql
'''
...
db.write_delta(f"DELTA_CTINDI_04_{params.sr_folio}", db.read_data("default", statement), "overwrite")
...
statement = query.get_statement(
    "CTINDI_DPSJL_TRN_0500_DBK.sql",
    DELTA_TABLA_NAME1 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_CTINDI_04_{params.sr_folio}",
    DELTA_TABLA_NAME2 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_CTINDI_02_{params.sr_folio}",
    sr_clave_ent_orig = '001'

)

db.write_delta(f"DELTA_CTINDI_05_{params.sr_folio}", db.sql_delta(statement), "overwrite")

if conf.debug:
    display(db.read_delta(f"DELTA_CTINDI_05_{params.sr_folio}"))
...
statement = query.get_statement(
    "CTINDI_DPSJL_TRN_0600_DBK_TRAN_GEN_ARCHIVO.sql",
    DELTA_TABLA_NAME1 = f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.DELTA_CTINDI_05_{params.sr_folio}",

)

db.write_delta(f"DELTA_CTINDI_06_{params.sr_folio}", db.sql_delta(statement), "overwrite")

if conf.debug:
    display(db.read_delta(f"DELTA_CTINDI_06_{params.sr_folio}"))

# COMMAND ----------

# DBTITLE 1,Validación de archivo vacío
if db.read_delta(f"DELTA_CTINDI_06_{params.sr_folio}").limit(1).count() > 0:
    ...
    file_manager.generar_archivo_ctindi(
        df_final=db.read_delta(f"DELTA_CTINDI_06_{params.sr_folio}"),  # DataFrame que se va a guardar
