WITH 
DS_200_DATOS AS (SELECT 
    FTC_FOLIO
    ,lpad(FTN_NUM_CTA_INVDUAL,10,'0') FTN_CTA_AFORE
    ,lpad(nvl(FTC_NSS_IMSS,'0'),11,'0')FTC_NSS_AFORE
    ,SUBSTR(FTC_RFC,12,1) FTC_DIG_VERI
    ,to_char(FTD_FEH_CRE, 'YYYYMMDD')FTD_FEH_CRE
    ,lpad(nvl(FTN_ID_ARCHIVO,'0'),7,'0') FTN_ID_ARCHIVO
    ,FTN_SUBPROCESO AS FCN_ID_SUBPROCESO
    ,lpad(FCN_ID_TIPO_SUBCTA,4,'0') FCN_ID_TIPO_SUBCTA
    ,lpad(nvl(FTN_NO_LINEA,'0'),8,'0') FNN_ID_REFERENCIA,
