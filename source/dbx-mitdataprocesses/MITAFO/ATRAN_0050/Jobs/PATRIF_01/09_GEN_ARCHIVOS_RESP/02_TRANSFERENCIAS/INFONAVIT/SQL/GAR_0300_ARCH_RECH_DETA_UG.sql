...
WHERE I.TMC_USO='MOTIVO DE RECHAZO PARA PROCESAR DE USO DE GARANTIA'
...
WHERE 1=1 
AND FTC_FOLIO_BITACORA = '#SR_FOLIO#'  --'202507211759137875'
AND FTC_ESTATUS_REGISTRO = 0
AND FTN_ID_SUBP = #SR_SUBPROCESO#  --368
)
SELECT 
A.TIPO_REGISTRO,
...
LPAD(' ',43,' ') AS FILLER6,
CASE WHEN BD_200.TMN_CVE_ITGY IS NULL THEN RPAD(' ',2,' ') ELSE LPAD(BD_200.TMN_CVE_ITGY,2,'0') END AS MOTIVO_RECHAZO,
LPAD(' ',8,' ') AS FILLER7,
CASE WHEN A.FTN_PERIODO_PAGO IS NULL THEN LPAD('0',6,'0') ELSE REPLACE(LTRIM(RTRIM(A.FTN_PERIODO_PAGO)),'.','') END AS FTN_PERIODO_PAGO,
LPAD(' ',12,' ') AS FILLER8
