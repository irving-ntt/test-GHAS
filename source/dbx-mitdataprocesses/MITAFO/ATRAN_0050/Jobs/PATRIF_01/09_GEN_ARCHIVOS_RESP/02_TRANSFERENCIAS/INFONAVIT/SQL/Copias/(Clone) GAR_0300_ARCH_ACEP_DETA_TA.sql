...
AS
(
SELECT TO_CHAR(TRUNC(ADD_MONTHS(CURRENT_DATE,1),'MM'),'YYYYMMDD') AS FECHA_MOVIENTO, 
1 AS CRUCE 
FROM DUAL
)
SELECT
...
ELSE   LPAD(
     LPAD(SUBSTR(TO_CHAR(A.FTN_NUM_AIVS_VIVI97_ULT), 1, INSTR(TO_CHAR(A.FTN_NUM_AIVS_VIVI97_ULT), '.') - 1),9,'0') 
   ||RPAD(SUBSTR(TRIM(TO_CHAR(A.FTN_NUM_AIVS_VIVI97_ULT)),INSTR(TRIM(TO_CHAR(A.FTN_NUM_AIVS_VIVI97_ULT)), '.') + 1, 6),6,'0')
          ,15,'0') END FTN_NUM_AIVS_VIVI97_ULT,      
CASE WHEN A.FTN_SALDO_VIVI97_ULT IS NULL THEN LPAD('0',15,'0')
    ELSE 
    LPAD(
    LPAD(SUBSTR(TO_CHAR(A.FTN_SALDO_VIVI97_ULT, 'FM9999999999.99')||'00', 1, INSTR(TO_CHAR(A.FTN_SALDO_VIVI97_ULT, 'FM9999999999.99')||'00', '.') - 1),13,'0') 
...
ELSE LPAD(
    LPAD(SUBSTR(TO_CHAR(A.FTN_NUM_APLI_INTE_VIVI92_ENV), 1, INSTR(TO_CHAR(A.FTN_NUM_APLI_INTE_VIVI92_ENV), '.') - 1),9,'0') 
   ||RPAD(SUBSTR(TRIM(TO_CHAR(A.FTN_NUM_APLI_INTE_VIVI92_ENV)),INSTR(TRIM(TO_CHAR(A.FTN_NUM_APLI_INTE_VIVI92_ENV)), '.') + 1, 6),6,'0')
    ,15,'0') END FTN_NUM_APLI_INTE_VIVI92_ENV, 
CASE WHEN A.FTN_SALDO_TOT_VIVI92 IS NULL THEN LPAD('0',15,'0')
   ELSE 
   LPAD(
    LPAD(SUBSTR(TO_CHAR(A.FTN_SALDO_TOT_VIVI92, 'FM9999999999.99')||'00', 1, INSTR(TO_CHAR(A.FTN_SALDO_TOT_VIVI92, 'FM9999999999.99')||'00', '.') - 1),13,'0') 
