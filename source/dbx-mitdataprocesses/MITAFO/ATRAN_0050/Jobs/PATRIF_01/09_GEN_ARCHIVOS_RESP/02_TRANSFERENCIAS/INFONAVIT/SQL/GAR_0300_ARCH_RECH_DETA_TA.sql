...
WHERE I.TMC_USO='MOTIVO DE RECHAZO PARA PROCESAR DE TRANSFERENCIA DE ACREDITADOS'
...
LPAD(' ',30,' ') AS FILLER5,
LPAD('0',15,'0') AS NUM_INTER_VIV97_ULT_APORT,
LPAD('0',15,'0') AS ULTIM_APOR_VIV97,
LPAD(' ',78,' ') AS FILLER6,
...
LPAD(' ',43,' ') AS FILLER7,
CASE WHEN BD_200.TMN_CVE_ITGY IS NULL THEN LPAD(' ',2,' ') ELSE LPAD(BD_200.TMN_CVE_ITGY,2,'0') END AS MOTIVO_DEVOL,
LPAD(' ',8,' ') AS FILLER8,
CASE WHEN A.FTN_PERIODO_PAGO IS NULL THEN LPAD(' ',6,' ') ELSE LPAD(REPLACE(LTRIM(RTRIM(A.FTN_PERIODO_PAGO)),'.',''),6,' ') END AS FTN_PERIODO_PAGO,
LPAD(' ',12,' ') AS FILLER9
