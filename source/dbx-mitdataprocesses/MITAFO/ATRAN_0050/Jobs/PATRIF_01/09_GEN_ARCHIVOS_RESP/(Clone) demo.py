...
# MAGIC Este notebook está diseñado para demostrar las funciones del framework.
# MAGIC ## 📌 Pasos principales:
# MAGIC 1. **Carga del Framework**: Se ejecuta un script de inicialización.
# MAGIC 2. **Definición y Validación de Parámetros**: Se definen los parámetros de entrada del proceso.
# MAGIC 3. **Carga de Configuraciones Locales**: Se establecen configuraciones generales.
# MAGIC 4. **Procesamiento de Datos**: Se ejecutan transformaciones y consultas sobre los datos.
# MAGIC 5. **Almacenamiento de Resultados**: Se guardan los resultados en la ubicación esperada.
# MAGIC ***
...
df = db.read_data("prueba", statement_demo_001)
display(df)
...
# MAGIC %md
# MAGIC # Listar tablas deltas creadas
...
# MAGIC %md
# MAGIC ## Verificar si la ejecucion asincrona finalizo

# COMMAND ----------

# DBTITLE 1,Verificar si la ejecucion asincrona finalizo
if execution.done():
    logger.info("La ejecución finalizó") # Forma correcta de evitar print
    logger.info(execution.result())  
else:
    logger.info("La ejecución sigue en curso")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Verificar si la ejecucion asincrona finalizo en bucle

# COMMAND ----------

# DBTITLE 1,Verificar si la ejecucion asincrona finalizo en bucle
import time
# Esperar a que termine la ejecución
while not execution.done():
    logger.info("⏳ La ejecución sigue en curso...")  # Mensaje de espera
    time.sleep(5)  # Esperar 5 segundos antes de volver a verificar

# Una vez que termina, imprimir el resultado
logger.info("✅ La ejecución finalizó")
logger.info(execution.result())  # Obtener el resultado cuando finalice

# COMMAND ----------

df_final = db.read_data("default",statement_demo_002)
display(df_final)
