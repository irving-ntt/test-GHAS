-- Query para extraer el detalle de archivo 70 desde tabla delta 
WITH RID AS 
( 
SELECT
...
-- RID.*
  ...
  CASE WHEN RID.SINDESMARCAR = 1 THEN NULL ELSE CAST(to_date(RID.FECHAMARCA, 'yyyyMMdd') AS DATE) END AS  FHD_FEH_MARCA,
  CASE WHEN RID.SINDESMARCAR = 1 THEN NULL ELSE  CAST(to_date(RID.FECHADESMARCA, 'yyyyMMdd') AS DATE)  END AS  FHD_FEH_DESMARCA,
