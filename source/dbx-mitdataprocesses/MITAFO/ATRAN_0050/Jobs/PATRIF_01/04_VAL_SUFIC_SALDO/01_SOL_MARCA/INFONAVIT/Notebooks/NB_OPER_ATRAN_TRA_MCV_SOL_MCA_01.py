...
#Query Extrae informacion de MARCA_DESMARCA_INFO
...
#Query Extrae informacion de VALOR_ACCION
...
db.write_delta(f"DELTA_TRA_DELTA_UPDATE_SALDOS_{params.sr_folio}", DLTstatement_005, "overwrite")

if conf.debug:
    display(db.read_delta(f"DELTA_TRA_DELTA_UPDATE_SALDOS_{params.sr_folio}"))



# COMMAND ----------

# ELIMINA REGISTROS DEL FOLIO BITACORA EN TABLA MARCA DESMARCA AUXILIAR
statement = query.get_statement(
...
# COMMAND ----------
    ...
    #UPDATE EN MARCA DESMARCA AUX

table_name = "CIERREN_DATAUX.TTCRXGRAL_MARCA_DESMARCA_INFO_AUX"

db.write_data(db.read_delta(f"DELTA_TRA_DELTA_UPDATE_SALDOS_{params.sr_folio}"), table_name, "default", "append")
