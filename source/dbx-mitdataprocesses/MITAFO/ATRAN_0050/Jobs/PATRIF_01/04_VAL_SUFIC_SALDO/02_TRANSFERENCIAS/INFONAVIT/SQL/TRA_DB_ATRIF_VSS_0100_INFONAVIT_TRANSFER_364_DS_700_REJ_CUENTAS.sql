WITH RankedData AS (
    ...
    AND (
        (NVL(c.SALDO_DISPONIBLE_AIVS, 0) <= 0 OR NVL(a.FTN_NUM_APLI_INTE_VIVI97_SOL, 0) = 0)
        AND (NVL(b.SALDO_DISPONIBLE_AIVS, 0) <= 0 OR NVL(a.FTN_NUM_APLI_INTE_VIVI92_SOL, 0) = 0) )
),
GeneratedColumns AS (
    SELECT 
...
RejectedData AS (
    ...
    FROM 
        GeneratedColumns
    WHERE 
        Bandera IS not NULL -- Reject condition
)
SELECT 
    FTN_NUM_CTA_INVDUAL,
    FTN_ID_SUBP,
