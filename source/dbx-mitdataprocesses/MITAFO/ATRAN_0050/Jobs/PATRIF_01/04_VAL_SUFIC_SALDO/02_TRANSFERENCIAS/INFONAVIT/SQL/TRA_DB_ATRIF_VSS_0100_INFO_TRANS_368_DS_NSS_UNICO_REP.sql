 SELECT 
    ...
    1 AS FTN_IND_SDO_DISP_VIV97,               -- Column 2: Rule: Set FTN_IND_SDO_DISP_VIV97 to 1
    d.FCN_ID_VALOR_ACCION,                     -- Column 3: Output column from DB_700_VALOR_ACCION
    '#sr_folio#' AS FTC_FOLIO_BITACORA,        -- Column 4: Rule: Pass FTC_FOLIO_BITACORA
    15 AS FCN_ID_TIPO_SUBCTA_97,               -- Column 5: Rule: Map FCN_ID_VALOR_ACCION to FCN_ID_TIPO_SUBCTA_97
    v.FTN_ID_SUBP,                             -- Column 6: Output column from CO_502_SUMREG
    NVL(v.FTN_SALDO_VIVI97_SOLI, 0) AS FTN_SALDO_VIVI97_SOLI, -- Column 7: Output column from CO_502_SUMREG
    NVL(v.FTN_NUM_APLI_INTE_VIVI97_SOL, 0) AS FTN_NUM_APLI_INTE_VIVI97_SOL, -- Column 8: Output column from CO_502_SUMREG
    NVL(d.FCN_VALOR_ACCION, 0) AS FCN_VALOR_ACCION, -- Column 9: Output column from DB_700_VALOR_ACCION
        CASE 
        WHEN NVL(v.FTN_NUM_APLI_INTE_VIVI97_SOL, 0) <= NVL(v.SALDO_DISPONIBLE_AIVS_VIV97, 0) 
        THEN NVL(v.FTN_NUM_APLI_INTE_VIVI97_SOL, 0) 
        ELSE NVL(v.SALDO_DISPONIBLE_AIVS_VIV97, 0) 
    END AS FTN_SDO_AIVS,                        -- Column 11: Rule: Calculate FTN_SDO_AIVS
    v.FTN_CONTA_SERV                          -- Column 10: Output column from CO_502_SUMREG
FROM 
    (
...
OR (FTN_MOTIVO_RECHAZO IS NULL))              -- Rule: Filter by FTC_ESTATUS_REGISTRO = 0
...
WHERE  -- Rule: Filter for unique NSS (CONTEO = 1)
    NVL(a.CONTEO, 0) = 1 