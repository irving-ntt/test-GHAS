...
SELECT
    ...
    CAST(NVL(L.FCN_VALOR_ACCION, 0) AS DECIMAL(18, 14)) AS FCN_VALOR_ACCION,
  ...
  WHERE FCN_ID_SIEFORE = 81
    ...
    AND FTC_NUM_CTA_INVDUAL IN (
      ...
      WHERE FTC_FOLIO = '#sr_folio#'
        ...
        ON ADD_MONTHS(TRUNC(a.FTD_FEH_PRESEN, 'MONTH'), 1) = TRUNC(L.FCD_FEH_ACCION)
        WHERE
           A.FTC_FOLIO = '#sr_folio#'
          AND ((FTN_MOTIVO_RECHAZO <> 93) AND   (FTN_MOTIVO_RECHAZO <> 254) AND  (FTN_MOTIVO_RECHAZO <> 375)  AND  (FTN_MOTIVO_RECHAZO <> 1178) OR (FTN_MOTIVO_RECHAZO IS NULL))
