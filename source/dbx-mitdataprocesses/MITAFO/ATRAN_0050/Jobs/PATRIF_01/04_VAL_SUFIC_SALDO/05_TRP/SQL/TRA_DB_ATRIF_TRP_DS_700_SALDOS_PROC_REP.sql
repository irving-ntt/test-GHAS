WITH tf_505_stagevars AS (
  SELECT
    *,
    -- vDiagnostico (numeric)
    CASE
      WHEN Diagnostico_97 IS NULL OR Diagnostico_08 IS NULL
        THEN CAST(COALESCE(Diagnostico_97, Diagnostico_08) AS INTEGER)
      WHEN Diagnostico_97 = Diagnostico_08
        THEN CAST(Diagnostico_97 AS INTEGER)
      WHEN Diagnostico_97 <> Diagnostico_08
        THEN 101
      ELSE 100
    END AS vDiagnostico,
    -- vMotivoRechazo (numeric mapping expanded to 104,100,101)
    CASE
      WHEN
        (CASE
           WHEN Diagnostico_97 IS NULL OR Diagnostico_08 IS NULL THEN CAST(COALESCE(Diagnostico_97, Diagnostico_08) AS INTEGER)
           WHEN Diagnostico_97 = Diagnostico_08 THEN CAST(Diagnostico_97 AS INTEGER)
