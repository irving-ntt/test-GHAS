...
statement = query.get_statement(
    "TRA_DB_ATRIF_TRP_DS_300_TRANFERENCIAS_REP.sql",
    sr_folio=params.sr_folio,
)

db.write_delta(f"DELTA_TRA_DB_ATRIF_TRP_DS_300_TRANFERENCIAS_{params.sr_folio}", db.read_data("default", statement), "overwrite")
if conf.debug:
    display(db.read_delta(f"DELTA_TRA_DB_ATRIF_TRP_DS_300_TRANFERENCIAS_{params.sr_folio}")) 
...
DS_400_RejInsSaldos = "DELTA_TRA_DS_400_RejInsSaldos_" + params.sr_folio
DS_400_NNS_UNICO = "DELTA_TRA_DS_400_NNS_UNICO_" + params.sr_folio


statement_003 = query.get_statement(
    "TRA_DB_ATRIF_TRP_FU_504_UNI_COMPLETA_REP.sql",
...
statement_003 = query.get_statement(
    "TRA_DB_ATRIF_TRP_DB_700_PRO_SUF_SALDOS_INS_REP.sql",
...
statement_003 = query.get_statement(
    "TRA_DB_ATRIF_TRP_DB_600_TRP_REP.sql",
 FU_504_UNI_COMPLETA=f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.{FU_504_UNI_COMPLETA}",
     sr_folio=params.sr_folio,
)
DLTstatement_003 = df = db.sql_delta(query=statement_003)
...
statement_003 = query.get_statement(
    "TRA_DB_ATRIF_TRP_DS_700_SALDOS_PROC_REP.sql",
 FU_504_UNI_COMPLETA=f"{SETTINGS.GENERAL.CATALOG}.{SETTINGS.GENERAL.SCHEMA}.{FU_504_UNI_COMPLETA}",
     sr_folio=params.sr_folio,
)
DLTstatement_003 = df = db.sql_delta(query=statement_003)
...
# ELIMINA REGISTROS DEL FOLIO BITACORA EN CIERREN_DATAUX.TTAFOTRAS_TRANS_REC_PORTA_AUX
statement = query.get_statement(
    "TRA_DB_DEL_TRANS_REC_PORTA_AUX.sql",
    SR_FOLIO=params.sr_folio,
    hints="/*+ PARALLEL(8) */",
)
execution = db.execute_oci_dml(
...
table_name = "CIERREN_DATAUX.TTAFOTRAS_TRANS_REC_PORTA_AUX"

db.write_data(db.read_delta(f"DELTA_TRA_DB_600_TRP_{params.sr_folio}"), table_name, "default", "append")

# COMMAND ----------

statement = query.get_statement(
    "TRA_DB_MERGE_TRANS_REC_PORTA_AUX.sql",
    SR_FOLIO=params.sr_folio,
    hints="/*+ PARALLEL(8) */",
)
execution = db.execute_oci_dml(
...
statement = query.get_statement(
    "TRA_DB_700_PRO_SUF_SALDOS_UPD_AUX_MERGE.sql",
    SR_FOLIO=params.sr_folio,
    hints="/*+ PARALLEL(8) */",
)
execution = db.execute_oci_dml(
